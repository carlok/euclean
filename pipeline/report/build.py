...
FACTS_HEADER = "<!-- generated by pipeline.report.build; do not edit by hand -->"


def key_facts(run, control, loop_run):
    """The numbers the executive summary quotes, emitted from the artifacts.

    The summary is prose and stays hand-written, but every figure in it drifted
    away from the run it described — six of them at once, because the narrative
    was updated after a re-run and the numbers were not. Generating this block
    and injecting it means the prose can be wrong about emphasis but not about
    arithmetic.
    """
    import glob
    ...
    L = [FACTS_HEADER, "", "| quantity | value |", "|---|---|"]
    L.append(f"| reference corpus, kernel-verified | {len(corpus)} |")
    L.append(f"| grid members | {stability.get('members', 0)} |")
    L.append(f"| grid theorems, kernel-verified | {grid_total} |")
    if spread:
        k, e = spread.get("kept", {}), spread.get("existential_free", {})
        L.append(f"| grid corpus size, min-max | {k.get('min')}-{k.get('max')} |")
        L.append(f"| grid existential-free, min-max | {e.get('min')}-{e.get('max')} |")
    if imp:
        L.append(f"| importance rank correlation, mean | {imp.get('mean_spearman')} |")
        L.append(
            f"| importance rank correlation, range | "
    ...
    if stages.get("T0") and stages.get("T1"):
        ...
        L.append(f"| max proof depth, T0 to T1 | {a['max_proof_depth']} to {b['max_proof_depth']} |")
    if ablation:
        sp = ablation.get("spread", {})
        L.append(
            f"| concepts separating from the ablation baseline | "
            f"{sp.get('separated')}/{sp.get('of')} |"
        )
    if conj:
        rec = conj.get("attempt_recall", {})
        L.append(
            f"| conjecture attempt recall on known statements | "
            f"{rec.get('recovered')}/{rec.get('known_statements_tried')} |"
        )
    L.append("")
    return "\n".join(L)


def inject_key_facts(text, block):
    """Replace the generated block in the summary, leaving the prose alone."""
    start = text.find(FACTS_HEADER)
    if start == -1:
        return text.rstrip() + "\n\n## Generated counts\n\n" + block
    end = text.find("\n## ", start)
    tail = text[end:] if end != -1 else ""
...
def main():
    ...
    if summary.exists():
        block = key_facts(args.run, args.control, args.loop)
        summary.write_text(inject_key_facts(summary.read_text(), block))
        print(f"refreshed the generated figures block in {summary}")
