...
**Scale.** 1904 kernel-verified consequences in the reference corpus, all
distinct after canonicalization, re-checkable in 9 seconds. A further 21,705
across the 45-member configuration grid.
...
The ranking *within* that set is not stable (mean rank correlation 0.41 across
490 member pairs, range −0.79 to 1.0). The measure identifies a robust set; it
does not robustly order it. Both halves of that sentence are results.
...
ranges from 84 to 1221 and existential-free content from 9 to 971. Any single
run's numbers are a sample, not a measurement. This is the main methodological
lesson and it applies retroactively to every number the first sprint reported.
